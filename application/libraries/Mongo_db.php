...
Class Mongo_db{
	...
	*/
	...
	{
		error_log('Called insert: ' . $collection . ' data: ' . print_r($insert,true));
		if (empty($collection))
		{
		...
		{
			error_log("Nothing to insert into Mongo collection or insert is not an array");
		}
		...
		try
		{
			...
			if (isset($insert['_id']))
			{
				error_log('Success!: ' . $insert['_id']);
				return ($insert['_id']);
			}
			else
			{
				error_log('BOO! couldnt insert');
				return (FALSE);
			}
		}
		catch (MongoCursorException $e)
		{
			if(isset($this->debug) == TRUE && $this->debug == TRUE)
			{
				error_log("Insert of data into MongoDB failed: {$e->getMessage()}");
			}
			else
			{
				error_log("Insert of data into MongoDB failed");
			}
		}
	}
	...
	*/
	...
	/**
	* // Row
	* same as get but returns the first record
	**/
	public function row($collection = "")
	{			
		if (empty($collection))
		{
		...
		{	
			...
			{
				return (!empty($returns)) ? $returns[0] : null;
			}
		}
		catch (MongoCursorException $e)
		{
	...
	*/
	public function update($collection = "", $options = array())
	{
		...
		try
		{
			...
			$save = $this->db->{$collection}->update($this->wheres, $this->updates, $options);
			error_log(print_r($this->wheres,true));
			error_log("SAVED TO MONGO? " . print_r($save,true));
			$this->_clear();
